...
if __name__ == "__main__":
    ...
    reg = stats.linregress(np.log10(regX)[idx], np.log10(regY)[idx])
    print "A-star %s: logy = %.4f + %.4f logx.  logy=0 at x=%.4f" % (doPrefilter, reg[1], reg[0], 10**(-reg[1]/reg[0]))
    xmod = np.arange(regX.min(), regX.max(), 0.0005)
    ymod = 10**(reg[1] + reg[0] * np.log10(xmod))
    plt.plot(xmod, ymod, "b-", alpha=0.75)
    plt.axvline(x=10**(-reg[1]/reg[0]), c='b', linestyle='--', alpha=0.5)
    ...
    idx = np.isfinite(np.log10(regY))
    reg = stats.linregress(np.log10(regX)[idx], np.log10(regY)[idx])
    print "M-star %s: logy = %.4f + %.4f logx.  logy=0 at x=%.4f" % (doPrefilter, reg[1], reg[0], 10**(-reg[1]/reg[0]))
    xmod = np.arange(regX.min(), regX.max(), 0.0005)
    ymod = 10**(reg[1] + reg[0] * np.log10(xmod))
    plt.plot(xmod, ymod, "r-", alpha=0.75)
    plt.axvline(x=10**(-reg[1]/reg[0]), c='r', linestyle='--', alpha=0.5)
