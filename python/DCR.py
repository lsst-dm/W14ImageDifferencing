...
def dcrPhosim(wavelen, zang, P=600.0, T=7.0, f=8.0):
    # Shown to be consistent with phoSim
    # Wavelength in microns, zd in degrees
    # Returns numbers in arcseconds

    def megaNStp(wavelen):
        x = (1./wavelen)**2.
        return 64.328 + (29498.1/(146.-x)) + (255.4/(41.-x))

    def megaN(wavelen, temp, pressure, wvp):
        nstp = megaNStp(wavelen)
    ...
    def dcr(wavelen, temp, pressure, wvp, zang):
        return 0.206265*(megaN(wavelen, temp, pressure, wvp) - megaN(.5, temp, pressure, wvp))*np.tan(np.pi*zang/180.)

    return dcr(wavelen, T, P, f, zang)
...
if __name__ == "__main__":
    ...
    if True:
        # Compare phoSim version with this one
        zds   = (0, 10, 20, 30, 40, 50)
        lcomp = 7501 * 10**-4  # For our M dwarf
        lref  = 5000 * 10**-4  # For comparison with photim
        
        dcr1  = []
        dcr2  = []
        for zd in zds:
            dcr1.append(dcrPhosim(lcomp, zd, P=600.0, T=7.0, f=8.0))
            r2a = refract(lcomp, zd*np.pi/180., P=600.0, T=7.0, f=8.0) * 180 / np.pi * 3600.
            r2b = refract(lref,  zd*np.pi/180., P=600.0, T=7.0, f=8.0) * 180 / np.pi * 3600.
            dcr2.append(r2a-r2b)
        plt.plot(zds, dcr1, label="phoSim")
        plt.plot(zds, dcr2, label="acb")
        plt.legend()
    ...
    sp05 = fig5.add_subplot(gs[3, 0])
    spz5 = fig5.add_subplot(gs[3, 5])
    ...
    spz5.yaxis.tick_right()
    spz5.yaxis.set_label_position("right")
    ...
    for sidx, sed in enumerate(seds):
        ...
        ax = fig5.add_subplot(gs[0, sidx+1], sharex=sp02)
        ax.plot(sed.wavelen, sed.fnu, lw=2)
        plt.setp(ax.get_xticklabels()+ax.get_yticklabels(), visible=False)
        ax.set_title(sNames[sidx], weight="bold", fontsize=16)
        ax.yaxis.set_major_formatter( fluxFormatter )
    ...
    for bidx, bp in enumerate(bps):
        ...
        if bidx == 3:
            ...
            ax = fig3.add_subplot(gs[bidx+1, 0], sharex=sp03)
            plt.setp(ax.get_xticklabels()+ax.get_yticklabels(), visible=False)
        ax.plot(bp.wavelen, bp.phi, "%s-" % (bColors[bidx]), lw=2)
        ax.set_ylabel(bNames[bidx], weight="bold", fontsize=16)
        ax.yaxis.set_major_formatter( fluxFormatter )
        ...
        if bidx == 3:
            ...
            ax = fig4.add_subplot(gs[bidx+1, 0], sharex=sp04)
            plt.setp(ax.get_xticklabels()+ax.get_yticklabels(), visible=False)
        ax.plot(bp.wavelen, bp.phi, "%s-" % (bColors[bidx]), lw=2)
        ax.set_ylabel(bNames[bidx], weight="bold", fontsize=16)
        ax.yaxis.set_major_formatter( fluxFormatter )
        ...
        if bidx == 3:
            ...
            ax = fig5.add_subplot(gs[bidx+1, 0], sharex=sp05)
            plt.setp(ax.get_xticklabels()+ax.get_yticklabels(), visible=False)
        ax.plot(bp.wavelen, bp.phi, "%s-" % (bColors[bidx]), lw=2)
        ax.set_ylabel(bNames[bidx], weight="bold", fontsize=16)
        ax.yaxis.set_major_formatter( fluxFormatter )
    ...
    for bidx, bp in enumerate(bps):
        for sidx, sed in enumerate(seds):
            ...
            offsets = []
            for zd in zds:
                offset = refract(wavelen*10**-3, zd * np.pi / 180., docorr=False) * 180. / np.pi * 3600.
                offsets.append(np.sum(offset * flux) / np.sum(flux))
            ...
            for zd in zds:
                offset  = refract(wavelen*10**-3, zd * np.pi / 180.) * 180. / np.pi * 3600.
                offsetg = refract(waveleng*10**-3, zd * np.pi / 180.) * 180. / np.pi * 3600.
            ...
            for zd in zds:
                offset  = refract(wavelen*10**-3, zd * np.pi / 180., docorr=False) * 180. / np.pi * 3600.
                offsetg = refract(waveleng*10**-3, zd * np.pi / 180., docorr=False) * 180. / np.pi * 3600.
                offsets.append(np.sum(offset * flux) / np.sum(flux) - np.sum(offsetg * fluxg) / np.sum(fluxg))
            ax.plot(zds, offsets, "r--", lw=2)
            ...
            for i in range(len(zds)):
                offset1 = refract(wavelen*10**-3, zds1[i] * np.pi / 180.) * 180. / np.pi * 3600.
                offset2 = refract(wavelen*10**-3, zds2[i] * np.pi / 180.) * 180. / np.pi * 3600.
            ...
            offsets = []
            for i in range(len(zds)):
                offset1 = refract(wavelen*10**-3, zds1[i] * np.pi / 180., docorr=False) * 180. / np.pi * 3600.
                offset2 = refract(wavelen*10**-3, zds2[i] * np.pi / 180., docorr=False) * 180. / np.pi * 3600.
                o1      = np.sum(offset1 * flux) / np.sum(flux)
                o2      = np.sum(offset2 * flux) / np.sum(flux)
                offsets.append(o1 - o2)
            ax.plot(zds, offsets, "r--", lw=2)
            ...
            offsets = []
            for zd in zds:
                offset = dcrPhosim(wavelen*10**-3, zd)
                offsets.append(np.sum(offset * flux) / np.sum(flux))
            if bidx == 2 and sidx == 4:
                ax = spz5
            else:
                ax = fig5.add_subplot(gs[bidx+1, sidx+1], sharey=spz5)
                plt.setp(ax.get_xticklabels()+ax.get_yticklabels(), visible=False)
            ax.plot(zds, offsets, "k-", lw=2)
            ax.axhline(y=0, c='k', linestyle='--')
    ...
    fig3.text(0.01, 0.825, r"$\frac{\int\ R(\lambda) f(\lambda) d\lambda}{\int\ f(\lambda) d\lambda}\ -\ \frac{\int\ R(\lambda) f(\lambda)_{G0V}\ d\lambda}{\int\ f(\lambda)_{G0V}\ d\lambda}$", weight="bold", fontsize=19)
    ...
    fig4.text(0.01, 0.825, r"$\frac{\int\ R(\lambda)_{LLC} f(\lambda) d\lambda}{\int\ f(\lambda) d\lambda}\ -\ \frac{\int\ R(\lambda)_{RHC} f(\lambda)\ d\lambda}{\int\ f(\lambda)\ d\lambda}$", weight="bold", fontsize=17)
    spz4.set_ylim(-1.01, .11)

    sp05.set_xlim(350, 900)
    plt.setp(sp05.get_yticklabels(), visible=False)
    plt.setp(sp05.get_xticklabels(), weight="bold", fontsize=11, rotation=45)
    sp05.set_xlabel("Wavelength (nm)", weight="bold", fontsize=13)
    spz5.set_xlim(0, 50)
    plt.setp(spz5.get_xticklabels(), weight="bold", fontsize=11)
    plt.setp(spz5.get_yticklabels(), weight="bold", fontsize=11)
    spz5.set_xlabel("ZD (deg)", weight="bold", fontsize=13)
    spz5.set_ylabel(r'$\Delta x $"', weight="bold", fontsize=15, rotation=360)
    fig5.text(0.01, 0.825, r"$\frac{\int\ [R(\lambda)- R(\lambda=500nm)]f(\lambda) d\lambda}{\int\ f(\lambda) d\lambda}$", weight="bold", fontsize=17)
    spz5.set_ylim(-1.01, .11)
    ...
    fig5.subplots_adjust(hspace=0.025, wspace=0.025)
    plt.show()
